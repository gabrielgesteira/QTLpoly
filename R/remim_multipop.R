...
#'   }
#'
#' @author Guilherme da Silva Pereira, \email{gdasilv@@ncsu.edu}, Getulio Caixeta Ferreira, \email{getulio.caifer@usp.br}, Gabriel de Siqueira Gesteira, \email{gdesiqu@ncsu.edu}
#'
#' @references
...
#'
#' @export remim_multipop
remim_multipop <- function (data, pheno.col = NULL, w.size = 15, sig.fwd = 0.01, 
          sig.bwd = 1e-04, score.null = NULL, d.sint = 1.5, polygenes = FALSE, 
          n.clusters = NULL, n.rounds = Inf, plot = NULL, verbose = TRUE, reml.package = c("varComp")) 
{
  # reml.package = match.arg(reml.package)
  ...
  for (p in 1:length(results)) {
    ...
    ## X <- matrix(1, length(Y))
    X = stats::model.matrix(DM ~ -1 + as.factor(Pop), data$pheno)
    if (!is.null(data$weights)) {
      weight <- data$weights[ind, pheno.col[p]]
    } else weight <- rep(1, length(ind))
    markers <- c(1:data$nmrk)
    if(reml.package == "asreml"){
      G <- lapply(markers, function(x) data$G[ind,ind,x] + diag(1e-5, length(ind)))
    } else G <- lapply(markers, function(x) data$G[ind,ind,x])
    ...
    while (temp["pv", which.max(temp["st", ])] <= sig.fwd & 
      ...
      while (temp["pv", which.max(temp["st", ])] <= sig.fwd) {
        ...
        if (polygenes) {
          Gstar <- apply(data$G[ind, ind, qtl.mrk], 
                         MARGIN = c(1, 2), sum)/length(qtl.mrk)
          Gstar[1:5, 1:5]
          full.mod0 <- varComp(Y ~ 1, varcov = list(Gstar), 
                               weights = weight/max(weight))
          control <- varComp.control(start = c(coef(full.mod0, 
                                                    what = "var.ratio"), 0))
          temp <- parSapply(cl, as.character(markers), 
                            function(x) {
                              m <- as.numeric(x)
                              full.mod <- varComp(Y ~ 1, varcov = list(Gstar, 
                                                                       data$G[ind, ind, m]), control = control, 
                                                  weights = weight/max(weight))
                              test <- varComp.test(full.mod, null = 1L)
                              c(st = as.numeric(test[[1]][[1]][[1]]$statistic), 
                                pv = as.numeric(test[[1]][[1]][[1]]$p.value))
                            })
        }
        else {
          if(reml.package == "asreml"){
            random = as.formula(paste0("~ ", paste0("vm(ind, qtl.vcv[[", 1:length(qtl.vcv), "]])", collapse = " + ")))
        ...
        if (length(qtl.mrk) > 1) {
          ...
          for (q in 1:length(qtl.mrk)) {
            if (length(qtl.mrk) > 1 & (length(qtl.mrk) - 
              ...
              if (polygenes) {
                Gstar <- apply(data$G[ind, ind, qtl.mrk0], 
                               MARGIN = c(1, 2), sum)/length(qtl.mrk0)
                Gstar[1:5, 1:5]
                full.mod0 <- varComp(Y ~ -1 + factor(data$pheno$Pop), varcov = list(Gstar), 
                                     weights = weight/max(weight))
                control <- varComp.control(start = c(coef(full.mod0, 
                                                          what = "var.ratio"), 0))
                temp <- parSapply(cl, as.character(markers.out), 
                                  function(x) {
                                    m <- as.numeric(x)
                                    full.mod <- varComp(Y ~ -1 + factor(data$pheno$Pop), varcov = list(Gstar, 
                                                                             data$G[ind, ind, m]), control = control, 
                                                        weights = weight/max(weight))
                                    test <- varComp.test(full.mod, null = 1L)
                                    c(st = as.numeric(test[[1]][[1]][[1]]$statistic), 
                ...
                if(reml.package == "asreml"){
                  random = as.formula(paste0("~ ", paste0("vm(ind, qtl.vcv[[", 1:length(qtl.vcv), "]])", collapse = " + ")))
                  full.mod0 = asreml(Y ~ -1 + factor(data$pheno$Pop), random, trace = F)
                  tau = full.mod0$vparameters[1:length(qtl.vcv)]
                } else{
                  withCallingHandlers(full.mod0 <- varComp(Y ~ -1 + factor(data$pheno$Pop), varcov = qtl.vcv, weights = weight/max(weight)), warnings = "supress")
                  tau <- full.mod0$parms
